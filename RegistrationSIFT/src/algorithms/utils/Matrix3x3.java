...
import algorithms.models.NoninvertibleModelException;
 ...
 * </pre>
 *
 * The class provides most of the methods as static variants for matrices passed
 * as double[]. They are always meant to be in a row after row sequence:
 *
 * <pre>
...
final public class Matrix3x3 implements Serializable {
	...
	public double m00 = 1, m01 = 0, m02 = 0, m10 = 0, m11 = 1, m12 = 0, m20 = 0, m21 = 0, m22 = 1;
	 ...
	 *          <i>M<sub>22<sub></i>
	 */
	public Matrix3x3(final double m00, final double m01, final double m02, final double m10, final double m11,
	 ...
	 * performing operations on the double[] directly, use the static methods of
	 * {@link Matrix3x3}.
	 *
	 * @param m
	 */
	...
	final static public double det(final double[] a) {
		...
		return a[0] * a[4] * a[8] + a[3] * a[7] * a[2] + a[6] * a[1] * a[5] - a[2] * a[4] * a[6] - a[5] * a[7] * a[0]
		    - a[8] * a[1] * a[3];
	}
	 ...
	 * @return
	 */
	final static public double det(final double m00, final double m01, final double m02, final double m10,
	 ...
	 * @return
	 */
	final static public float det(final float m00, final float m01, final float m02, final float m10, final float m11,
	...
	final static public double[] createInverse(final double m00, final double m01, final double m02, final double m10,
	    final double m11, final double m12, final double m20, final double m21, final double m22)
		...
		return new double[] { (m11 * m22 - m12 * m21) / det, (m02 * m21 - m01 * m22) / det, (m01 * m12 - m02 * m11) / det,
		    (m12 * m20 - m10 * m22) / det, (m00 * m22 - m02 * m20) / det, (m02 * m10 - m00 * m12) / det,
		    (m10 * m21 - m11 * m20) / det, (m01 * m20 - m00 * m21) / det, (m00 * m11 - m01 * m10) / det };
	}

	final static public float[] createInverse(final float m00, final float m01, final float m02, final float m10,
		...
		return new float[] { (m11 * m22 - m12 * m21) / det, (m02 * m21 - m01 * m22) / det, (m01 * m12 - m02 * m11) / det,
		    (m12 * m20 - m10 * m22) / det, (m00 * m22 - m02 * m20) / det, (m02 * m10 - m00 * m12) / det,
		    (m10 * m21 - m11 * m20) / det, (m01 * m20 - m00 * m21) / det, (m00 * m11 - m01 * m10) / det };
	}
