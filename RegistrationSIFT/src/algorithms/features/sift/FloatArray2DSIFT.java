 ...
 * </pre>
 *
 * NOTE: The SIFT-method is protected by U.S. Patent 6,711,293: "Method and
...
public class FloatArray2DSIFT extends FloatArray2DFeatureTransform<FloatArray2DSIFT.Param> {
	static public class Param implements Serializable {
		...
		private static final long serialVersionUID = 7826874430970391999L;
		...
		public boolean equals(final Param p) {
			return (fdSize == p.fdSize) && (fdBins == p.fdBins) && (maxOctaveSize == p.maxOctaveSize)
			    && (minOctaveSize == p.minOctaveSize) && (steps == p.steps) && (initialSigma == p.initialSigma);
		}
	 ...
	 *          {@link Param#initialSigma} img must be a 2d-array of float values
	 *          in range [0.0f, ..., 1.0f]
	 */
	@Override
	final public void init(FloatArray2D src) {
		// estimate the number of octaves needed using a simple while loop instead
		// of ld
		int o = 0;
	 ...
	 * location, the regions size is defined by ( FEATURE_DESCRIPTOR_SIZE * 4 )^2
	 * ( 4x4 subregions )
	 *
	 * @param c
	 ...
	 *          orientation [-&pi; ... &pi;]
	 */
	private float[] createDescriptor(final double[] c, final int o, final double octave_sigma, final double orientation) {
		...
		for (int y = fdWidth - 1; y >= 0; --y) {
			final double ys = (y - 2.0 * p.fdSize + 0.5) * octave_sigma; // !< scale y
			                                                             // around 0,0
			for (int x = fdWidth - 1; x >= 0; --x) {
				final double xs = (x - 2.0 * p.fdSize + 0.5) * octave_sigma; // !< scale
				                                                             // x around
				                                                             // 0,0
	 ...
	 */
	final protected void processCandidate(final double[] c, final int o, final List<Feature> features) {
		...
		// feature
		final FloatArray2D gaussianMask = Filter.createGaussianKernelOffset(octave_sigma * 1.5, c[0] - Math.floor(c[0]),
		    c[1] - Math.floor(c[1]), false);
		...
		for (int i = 0; i < gradientROI[0].data.length; ++i) {
			final int bin = Math.max(0,
			    Math.min(ORIENTATION_BINS1, (int) ((gradientROI[1].data[i] + Math.PI) / ORIENTATION_BIN_SIZE)));
			histogram_bins[bin] += gradientROI[0].data[i];
		}

		// find the dominant orientation and interpolate it with respect to its two
		// neighbours
		int max_i = 0;
		...
		/**
		 * interpolate orientation estimate the offset from center of the parabolic
		 * extremum of the taylor series through env[1], derivatives via central
		 * difference and laplace
		 */
		...
		features.add(new Feature(octave_sigma * scale, orientation, new double[] { c[0] * scale, c[1] * scale },
		    // new double[]{ ( c[ 0 ] + 0.5f ) * scale - 0.5f, ( c[ 1 ] + 0.5f ) *
		    // scale - 0.5f },
		    createDescriptor(c, o, octave_sigma, orientation)));

		/**
		 * check if there is another significant orientation ( > 80% max ) if there
		 * is one, duplicate the feature and
		 */
		for (int i = 0; i < ORIENTATION_BINS; ++i) {
			if (i != max_i && (max_i + 1) % ORIENTATION_BINS != i && (max_i - 1 + ORIENTATION_BINS) % ORIENTATION_BINS != i
	...
	public Vector<Feature> run(final int max_size) {
		...
		// System.out.println( features.size() + " candidates processed in all
		// octaves" );
		return features;
	}
	 ...
	 */
	/*
	 * public static Vector< PointMatch > createMatches( final List< Feature >
	 * fs1, final List< Feature > fs2, final float rod ) { final Vector<
	 * PointMatch > matches = new Vector< PointMatch >();
	 * 
	 * for ( final Feature f1 : fs1 ) { Feature best = null; double best_d =
	 * Double.MAX_VALUE; double second_best_d = Double.MAX_VALUE;
	 * 
	 * for ( final Feature f2 : fs2 ) { final double d = f1.descriptorDistance( f2
	 * ); if ( d < best_d ) { second_best_d = best_d; best_d = d; best = f2; }
	 * else if ( d < second_best_d ) second_best_d = d; } if ( best != null &&
	 * second_best_d < Double.MAX_VALUE && best_d / second_best_d < rod )
	 * matches.addElement( new PointMatch( new Point( new double[] { f1.location[
	 * 0 ], f1.location[ 1 ] } ), new Point( new double[] { best.location[ 0 ],
	 * best.location[ 1 ] } ), ( f1.scale + best.scale ) / 2.0f ) ); }
	 * 
	 * // now remove ambiguous matches for ( int i = 0; i < matches.size(); ) {
	 * boolean amb = false; final PointMatch m = matches.get( i ); final double[]
	 * m_p2 = m.getP2().getL(); for ( int j = i + 1; j < matches.size(); ) { final
	 * PointMatch n = matches.get( j ); final double[] n_p2 = n.getP2().getL(); if
	 * ( m_p2[ 0 ] == n_p2[ 0 ] && m_p2[ 1 ] == n_p2[ 1 ] ) { amb = true;
	 * matches.removeElementAt( j ); } else ++j; } if ( amb )
	 * matches.removeElementAt( i ); else ++i; } return matches; }
	 * 
	 */
	/**
	...
	/*
	 * public static Vector< PointMatch > createMatches( final List< Feature >
	 * fs1, final List< Feature > fs2, final double max_sd, final AbstractModel< ?
	 * > model, final double max_id, final double rod ) { final Vector< PointMatch
	 * > matches = new Vector< PointMatch >(); final double min_sd = 1.0 / max_sd;
	 * 
	 * final int size = fs2.size(); final int size_1 = size - 1;
	 * 
	 * for ( final Feature f1 : fs1 ) { Feature best = null; double best_d =
	 * Double.MAX_VALUE; double second_best_d = Double.MAX_VALUE;
	 * 
	 * int first = 0; int last = size_1; int s = size / 2 + size % 2; if ( max_sd
	 * < Double.MAX_VALUE ) { while ( s > 1 ) { Feature f2 = fs2.get( last ); if (
	 * f2.scale / f1.scale < min_sd ) last = Math.max( 0, last - s ); else last =
	 * Math.min( size_1, last + s ); f2 = fs2.get( first ); if ( f2.scale /
	 * f1.scale < max_sd ) first = Math.max( 0, first - s ); else first =
	 * Math.min( size_1, first + s ); s = s / 2 + s % 2; } //System.out.println(
	 * "first = " + first + ", last = " + last + ", first.scale = " + fs2.get(
	 * first ).scale + ", last.scale = " + fs2.get( last ).scale +
	 * ", this.scale = " + f1.scale ); }
	 * 
	 * //for ( Feature f2 : fs2 )
	 * 
	 * for ( int i = first; i <= last; ++i ) { final Feature f2 = fs2.get( i );
	 * final double d = f1.descriptorDistance( f2 ); if ( d < best_d ) {
	 * second_best_d = best_d; best_d = d; best = f2; } else if ( d <
	 * second_best_d ) second_best_d = d; } if ( best != null && second_best_d <
	 * Double.MAX_VALUE && best_d / second_best_d < rod ) // not weighted //
	 * matches.addElement( // new PointMatch( // new Point( // new double[] {
	 * f1.location[ 0 ], f1.location[ 1 ] } ), // new Point( // new double[] {
	 * best.location[ 0 ], best.location[ 1 ] } ) ) ); // weighted with the
	 * features scale matches.addElement( new PointMatch( new Point( new double[]
	 * { f1.location[ 0 ], f1.location[ 1 ] } ), new Point( new double[] {
	 * best.location[ 0 ], best.location[ 1 ] } ), ( f1.scale + best.scale ) /
	 * 2.0f ) ); } // now remove ambiguous matches for ( int i = 0; i <
	 * matches.size(); ) { boolean amb = false; final PointMatch m = matches.get(
	 * i ); final double[] m_p2 = m.getP2().getL(); for ( int j = i + 1; j <
	 * matches.size(); ) { final PointMatch n = matches.get( j ); final double[]
	 * n_p2 = n.getP2().getL(); if ( m_p2[ 0 ] == n_p2[ 0 ] && m_p2[ 1 ] == n_p2[
	 * 1 ] ) { amb = true; //System.out.println( "removing ambiguous match at " +
	 * j ); matches.removeElementAt( j ); } else ++j; } if ( amb ) {
	 ...
	 * @param rs
	 */
	public static double[] featureSizeHistogram(final Vector<Feature> features, final double min, final double max,
	    final int bins) {
