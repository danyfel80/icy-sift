 ...
 * Based on the implementation of Stephan Saalfeld <saalfeld@mpi-cbg.de> 
...
public class SIFT extends FeatureTransform< FloatArray2DSIFT >
{
   ...
   * @param seq Sequence to extract features from.
  ...
  final public void extractFeatures( final Sequence seq, final Collection< Feature > features )
  {
    ...
    if ( maxSize < seq.getSizeX() || maxSize < seq.getSizeY() )
    {
      ...
      Sequence seqscaled = SequenceUtil.scale(seq, Math.round(seq.getSizeX()*scale), Math.round(seq.getSizeY()*scale));
