...
/**
 * Find the optimal similarity transformation between 2 images using Procrutes
 * analysis Only in 2D
 * 
...
public class SimilarityTransform {
	...
	// Algorithm RANSAC
	static public double[] ransac(List<PointMatch> candidates, final List<PointMatch> inliers, int nbIter, int nbrand,
	    double maxEpsilon) {
	...
	static public double[] ProcrustesAnalyze(List<PointMatch> candidates) {
		...
		for (int i = 0; i < N; i++) {
			...
			// Vector Y contains the coordinates of point correspondances from 2nd
			// image
			Y[2 * i] = l2[0];
			Y[2 * i + 1] = l2[1];
