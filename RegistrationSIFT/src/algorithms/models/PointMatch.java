 ...
 * 
 *         Attention!!: The world coordinates and weights are not used in the
 *         RegistrationSIFT plugin in Icy
 */
public class PointMatch implements Serializable {
	...
	/**
	 * Get the last weights element and remove it from the list. In case that only
	 * one element is in the list, the element is not removed but set to 1.0.
	 *
	 * @return
	 */
	 ...
	 * only one element is in the list, the element is not removed but set to 1.0.
	 *
	 * @return
	 */
	 ...
	 *          Array of weights
	 * @param strength
	 *          how much should {@link #apply(Model, double)} affect {@link #p1}
	 */
	public PointMatch(final Point p1, final Point p2, final double[] weights, final double strength) {
		this.p1 = p1;
		this.p2 = p2;
	 ...
	 * Constructor
	 *
	 * Create a {@link PointMatch} with an Array of weights. The Array of weights
	 * will be copied.
	 *
	 * @param p1
	 *          Point 1
	 ...
	 *          Array of weights
	 */
	public PointMatch(final Point p1, final Point p2, final double[] weights) {
		this.p1 = p1;
		this.p2 = p2;
	 ...
	 *          Weight
	 */
	public PointMatch(final Point p1, final Point p2, final double weight) {
		this.p1 = p1;
		this.p2 = p2;
	 ...
	 *          how much should {@link #apply(Model, double)} affect {@link #p1}
	 */
	public PointMatch(final Point p1, final Point p2, final double weight, final double strength) {
		this.p1 = p1;
		this.p2 = p2;
	 ...
	 */
	/*
	 * public void apply( final CoordinateTransform t ) { p1.apply( t ); }
	 */
	...
	/**
	 * Apply a {@link CoordinateTransform} to {@link #p1} a {@link Collection} of
	 * {@link PointMatch PointMatches}, update their distances.
	 *
	 * @param matches
	 * @param t
	 */
	/*
	 * static public void apply( final Collection< ? extends PointMatch > matches,
	 * final CoordinateTransform t ) { for ( final PointMatch match : matches )
	 * match.apply( t ); }
	 */

	/**
	 * Flip all {@link PointMatch PointMatches} from {@linkplain Collection
	 * matches} symmetrically and fill {@linkplain Collection flippedMatches} with
	 * them, weights remain unchanged.
	 *
	 * @param matches
	 *          original set
	 * @param flippedMatches
	 *          result set
	 */
	final public static void flip(final Collection<PointMatch> matches, final Collection<PointMatch> flippedMatches) {
	...
	final public static void cloneSourcePoints(final Collection<PointMatch> matches,
	...
	final public static void cloneTargetPoints(final Collection<PointMatch> matches,
	    final Collection<Point> targetPoints) {
		for (final PointMatch m : matches)
