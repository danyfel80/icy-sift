...
/**
 * Plugin for SIFT points registration.
 * Based on SIFT_ExtractPointRoi class developed by Stephan Saalfeld <saalfeld@mpi-cbg.de>
...
public class SIFTRegistration extends EzPlug implements EzStoppable{
   ...
   * only if the Euclidean distance to the nearest neighbor is significantly
   * smaller than that to the next nearest neighbor. 
   * Increase of there is a large deformation between 2 images.
   */
  private EzVarFloat inClosestToNextRatio;
  /**
   * Add spatial constraint in preliminary matching.
   * Uncheck in case of 2 images with different sizes.
   */
  private EzVarBoolean inIsSpatialConstraint;
  /**
   * Spatial constraint.
   * Correspondence candidates have to be in a certain region around the 
   * location of the keypoint considered.
   */
  private EzVarInteger inSpatialConstraint;
  /**
   * Maximal Alignment Error.
   * After transformation, the distance between 2 points has to be smaller
   * than this value to be considered inlier
   */
  private EzVarFloat inMaxAlignErr;
  /**
   * Number of random inliers.
   * The number of random correspondent pairs selected randomly by RANSAC in
   * each iteration.
   */
  ...
  protected void initialize() {
    ...
    inMaxResolution.setToolTipText(
        ...
        "set to image size to consider all the smallest " +
        "details of image");
    ...
    inClosestToNextRatio.setToolTipText("Correspondence candidates from local descriptor matching are accepted only if the Euclidean distance to the nearest neighbour is significantly smaller than that to the next nearest neighbour. Increase of there is a large deformation between 2 images");
    
    inSpatialConstraint = new EzVarInteger("Spatial constraint", p.sc , 0, 1000, 5);
    inSpatialConstraint.setToolTipText("Correspondance candidates have to be in a certain region around the location of the keypoint considered");
    ...
    EzGroup groupDescriptor = new EzGroup("Feature Descriptor", inMaxResolution, inMinResolution, inFeatDescSize, inFeatDescOBinSize, inClosestToNextRatio, inIsSpatialConstraint, inSpatialConstraint);
    super.addEzComponent(groupDescriptor);

    
    inMaxAlignErr = new EzVarFloat("Maximal Alignment Error", p.maxEpsilon, 5.0f, 100.0f, 1.0f);
    
    inSpatialConstraint.setToolTipText("After transformation, the distance between 2 points has to be smaller than this value to be considered inliers");
    
    inRandomInliersNum = new EzVarInteger("Number of random inliers", p.NumInliers, 4, 10, 1);
    inRandomInliersNum.setToolTipText("Number of random correspondant pairs selected randomly by RANSAC in each iteration");
    ...
    inIsDebug.setToolTipText("Execute the plugin in debug mode. All the intermediate images will be displayed, as well as the debug log.");
...
//		FloatArray2D fa1 = ImageArrayConverter.SequenceToFloatArray2DNormalize(seq1);
//		Sequence seq1gray = ImageArrayConverter.FloatArray2DtoSequence(fa1);
//		addSequence(seq1gray);
//		FloatArray2D fa2 = ImageArrayConverter.SequenceToFloatArray2DNormalize(seq2);
...
//    }
    ...
    if (stopFlag) {endExecution(); return;}
